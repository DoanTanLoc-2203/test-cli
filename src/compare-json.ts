...
function compareJson(obj1: any, obj2: any, path: string = ""): string[] {
  const diffs: string[] = [];
  ...
  for (const key of allKeys) {
    ...
    if (val1 === undefined) {
      diffs.push(
        `🟢 Key missing in first JSON: ${currentPath}, value in second = ${JSON.stringify(
          val2
        )}`
      );
    } else if (val2 === undefined) {
      diffs.push(
        `🔴 Key missing in second JSON: ${currentPath}, value in first = ${JSON.stringify(
          val1
        )}`
      );
    } else if (
      ...
      diffs.push(
        `⚠️ Difference at ${currentPath}: first = ${JSON.stringify(
          val1
        )}, second = ${JSON.stringify(val2)}`
      );
    }
  }
...
function runCompare(file1: string, file2: string) {
